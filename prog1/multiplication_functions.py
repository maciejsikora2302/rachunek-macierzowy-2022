...
def mat_mul_strassenR(A, B, treshold = 4):
    ...
    if n <= treshold:
        ...
        aResult = add(a11, a22)
        bResult = add(b11, b22)
        p1, _ = mat_mul_strassenR(aResult, bResult)  # p1 = (a11+a22) * (b11+b22)

        aResult = add(a21, a22)  # a21 + a22
        p2, _ = mat_mul_strassenR(aResult, b11)  # p2 = (a21+a22) * (b11)

        bResult = subtract(b12, b22)  # b12 - b22
        p3, _ = mat_mul_strassenR(a11, bResult)  # p3 = (a11) * (b12 - b22)

        bResult = subtract(b21, b11)  # b21 - b11
        p4, _ = mat_mul_strassenR(a22, bResult)  # p4 = (a22) * (b21 - b11)

        aResult = add(a11, a12)  # a11 + a12
        p5, _ = mat_mul_strassenR(aResult, b22)  # p5 = (a11+a12) * (b22)

        aResult = subtract(a21, a11)  # a21 - a11
        bResult = add(b11, b12)  # b11 + b12
        p6, _ = mat_mul_strassenR(aResult, bResult)  # p6 = (a21-a11) * (b11+b12)

        aResult = subtract(a12, a22)  # a12 - a22
        bResult = add(b21, b22)  # b21 + b22
        p7, _ = mat_mul_strassenR(aResult, bResult)  # p7 = (a12-a22) * (b21+b22)
...
def mat_mul_strassen(A, B):
    ...
    for i in range(n):
        for j in range(n):
            APrep[i][j] = A[i][j]
            BPrep[i][j] = B[i][j]
    CPrep, _ = mat_mul_strassenR(APrep, BPrep)
